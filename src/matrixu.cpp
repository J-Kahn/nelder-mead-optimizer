...
double cov(matrix mat1, matrix mat2){
  ...
  matrix cv = (dm1.t()*dm2)/((double) dm1.nrows());
  double ccv = cv(0);
...
matrix diag(const matrix& v)
{
  ...
  /*else
  {
    throw MyException("Parameter for diag must be a vector");
  }*/
  return res;
}
...
matrix diags(const matrix& v)
{
  ...
  {
    throw MyException("Matrix for diagonals must be square");
  }*/
  return res;
}
...
double det(const matrix& a)
{
  ...
  else
  {
    ...
    throw MyException("matrix must be square");
  }*/
  return d;
}
...
/*double determinant(const matrix a,int k)
{
  s=1,det=0;
     ...
     for (c=0;c<k;c++)
       {
        ...
        for (i=0;i<k;i++)
          ...
          det=det + s * (a.p[c*rows] * determinant(b,k-1));
          s=-1 * s;
          }
    }
...
/*matrix inv(matrix a)
{
  ...
  if (rows == cols && d != 0)
  {
    ...
    {
      // this is a matrix of 3 x 3 or larger
      // calculate inverse using gauss-jordan elmination
      ...
      for (int c = 0; c < cols; c++)
      {
        ...
        // elminate non-zero values on the other rows at column c
        for (int r = 0; r < rows; r++)
        {
          if(r != c)
          {
  ...
  else
  {
    ...
    {
      throw MyException("determinant of matrix is zero");
    }
  }
  return res;
}*/
